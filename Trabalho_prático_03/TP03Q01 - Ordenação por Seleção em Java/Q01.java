...
class Lista{
    ...
    //retorna a quantidade de movimentacoes feitas a ordenacao
    public int getMovimentacoes(){
        return movimentacoes;
    }


    //Ordenacao por selecao dos elementos da lista com chave PaisDeOrigem
    public void ordenar(Log log){
        ...
        for (int i=0; i<(n - 1); i++) {
            ...
            for (int j=(i + 1); j < n; j++){
                log.comp++;
                if (array[j].getPaisDeOrigem().compareTo(array[menor].getPaisDeOrigem()) < 0){
                    menor = j;
                }
                log.comp++;
                if (array[j].getPaisDeOrigem().compareTo(array[menor].getPaisDeOrigem()) == 0){
...
public class Q01{
    ...
    public static void main(String[] args) throws Exception {
        ...
        Arq.openWrite("matricula selecao.txt", "UTF-8");
