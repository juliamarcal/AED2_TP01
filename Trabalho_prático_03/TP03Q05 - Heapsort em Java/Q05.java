...
class Lista{
    ...
    //Ordenacao por heapSort dos elementos da lista com chave formato
    ...
    //construcao do heap
    public void construir(int tamHeap, Log log){
        for(int i = tamHeap; i > 1 && isMaior(array[i], array[i/2], log) == true; i /= 2){
            swap(i, i/2, log);
        }
    }

    //reconstrucao do heep
    public void reconstruir(int tamHeap, Log log){
        int i = 1;
        while(i <= (tamHeap/2)){
            int filho = getMaiorFilho(i, tamHeap, log);
            if(isMaior(array[filho], array[i], log)==true){
                swap(i, filho, log);
                i = filho;
            }else{
                i = tamHeap;
    ...
    public int getMaiorFilho(int i, int tamHeap, Log log){
        int filho;
        if (2*i == tamHeap || isMaior(array[2*i], array[2*i+1], log)==true){
           filho = 2*i;
        } else {
           filho = 2*i + 1;
        }
    ...
    //compara se o elemento i e maior que o elemento j com base na chave formato e, se der empate, na chave nome
    boolean isMaior(Serie i, Serie j, Log log){
        log.comp++;
        return(i.getFormato().compareTo(j.getFormato()) > 0 || ((i.getFormato().compareTo(j.getFormato()) == 0) && i.getNome().compareTo(j.getNome()) > 0));
    }
...
public class Q05{
    ...
    public static void main(String[] args) throws Exception {
        ...
        Arq.openWrite("matricula heapsort.txt", "UTF-8");
