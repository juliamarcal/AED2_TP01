...
class Lista{
    ...
    //Ordenacao por MergeSort dos elementos da lista com chave numEpisodios
    public void ordenar(int esq, int dir, Log log){

        if (esq < dir){
            int meio = (esq + dir) / 2;
            ordenar(esq, meio, log);
            ordenar(meio+1, dir, log);
            intercalar(esq, meio, dir, log);
        }
    ...
    public void intercalar(int esq, int meio, int dir, Log log){
        int n1, n2, i, j, k;
        ...
        Serie aux = new Serie();
        a1[i] = a2[j] = aux;
        aux.setNumEpisodios(0x7FFFFFFF);
  
        
        //Intercalacao propriamente dita
        for(i = j = 0, k = esq; k <= dir; k++){
            log.mov++;
           array[k] = (isMaior(a2[j], a1[i], log)==true) ? a1[i++].clone() : a2[j++].clone();
    ...
    //compara se o elemento i e maior que o elemento j com base na chave numEpisodios e, se der empate, na chave nome
    boolean isMaior(Serie i, Serie j, Log log){
        log.comp++;
        return((i.getNumEpisodios() > j.getNumEpisodios()) || ((i.getNumEpisodios() == j.getNumEpisodios()) && i.getNome().compareTo(j.getNome()) > 0));
    }
...
public class Q09{
    ...
    public static void main(String[] args) throws Exception {
        ...
        Arq.openWrite("matricula mergesort.txt", "UTF-8");
