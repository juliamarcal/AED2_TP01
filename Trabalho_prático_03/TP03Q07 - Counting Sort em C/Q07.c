...
// ( (strcmp(array[j].num_temporadas, tmp.num_temporadas)>0) || ( (strcmp(array[j].num_temporadas, tmp.num_temporadas)==0) && (strcmp(array[j].nome, tmp.nome)>0) ) )
//ordenar pelo metodo countingSort com a chave numeroDeTemporadas
void ordenar(){
...
void ordenarEmpate() {
    for (int i = 0; i < (n - 1); i++) {
        int menor = i;
        for (int j = (i + 1); j < n; j++){
            if ((array[menor].num_temporadas == array[j].num_temporadas) && (strcmp(array[menor].nome, array[j].nome) > 0)){
                menor = j;
            }
        }
        Serie temp = array[menor];
        array[menor] = array[i];
        array[i] = temp;
        }
}
