...
class LNomes{
    ...
    //inserir elementos no inicio da lista e descolca os demais elementos para o fim
    public void inserirInicio(String item, Log log) {
		if (n < arrayNomes.length) {
			// Desloca elementos para o fim do array
			for (int i = n; i > 0; i--)
            arrayNomes[i] = arrayNomes[i - 1];
    ...
    //pesquisa sequencial
    public boolean pesquisar(Object item, Log log) {
		for (int i = 0; i < n; i++){
            log.comp++;
			if (arrayNomes[i].equals(item)){
				return true;
            }
        }
		return false;
...
public class Q03{
    ...
    public static void main(String[] args) {
        ...
        for(int j=0; j<i; j++){
            nomes.inserirInicio(series[j].getNome(), log);
        }

        //efetuar a pesquisa sequencial
        entrada = MyIO.readLine();
        ...
        Arq.openWrite("matricula sequencial.txt", "UTF-8");
